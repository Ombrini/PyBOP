...
class PlotlyBackend(PlotBackend):
    ...
    def create_figure(self, title = None, xaxis_title = None, yaxis_title = None, traces = None, style = None):
        ...
        fig = self.plotly_manager.goFigure(data=traces, layout=layout)
        return fig
    ...
    def make_subplots(self, axes: list[_AxisData], title=None, axis_titles_x: list[str] | str = None, axis_titles_y: list[str] | str = None, style=None):
        ...
        for i, ax in enumerate(axes):
            if isinstance(axis_titles_x, str):
                fig.update_xaxes(title_text=axis_titles_x, row=ax.row, col=ax.col)
            elif isinstance(axis_titles_x, list) and i < len(axis_titles_x):
                fig.update_xaxes(title_text=axis_titles_x[i], row=ax.row, col=ax.col)
            if isinstance(axis_titles_y, str):
                fig.update_yaxes(title_text=axis_titles_y, row=ax.row, col=ax.col)
            elif isinstance(axis_titles_y, list) and i < len(axis_titles_y):
                fig.update_yaxes(title_text=axis_titles_y[i], row=ax.row, col=ax.col)
    ...
    def contour_plot(self, x, y, z, colorscale="viridis"):
        ...
        return self.plotly_manager.goContour(x=x, y=y, z=z, colorscale=colorscale, connectgaps=True) 

    def _get_line_options(style, opts):
        linestyle = style.get("linestyle", "solid")
        color = style.get("color")
        opts["line"] = dict(
            width = style.get("linewidth", 4),
            dash = LINESTYLE_MAP.get(linestyle, "solid")
        )
    ...
    def fill_plot(self, x, y, color=None, label=None):
        ...
        return self.plotly_manager.goScatter(x=x, y=y, fill="toself", mode="text", showlegend=False, **opts)

    def fill_between_plot(self, x, y_upper, y_lower, color):
        
        return self.plotly_manager.goScatter(
            x=x + x[::-1],
            y=y_upper + y_lower[::-1],
            fill="toself",
    ...
    def histogram_plot(self, x, name, style=None):
        ...
        return self.plotly_manager.goHistogram(x=x, name=name, opacity = style.get("alpha"))
    ...
    def line_plot(self, x=None, y=None, label=None, style = None):
        ...
        if marker.lower() != "none":
            ...
            if markeredgecolor is not None:
                opts["marker"].update(line_color = markeredgecolor, line_width=1 or markeredgewidth)
    ...
    def scatter_plot(self, x, y, colors, labels=None, colorscale="Greys"):
        ...
        return self.plotly_manager.goScatter(x=x, y=y, **opts)
    ...
    def show_table(self, header, values, title):
        ...
        fig = self.plotly_manager.goFigure(
            data=[
                self.plotly_manager.goTable(
